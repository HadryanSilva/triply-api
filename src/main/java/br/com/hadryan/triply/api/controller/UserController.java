...
public class UserController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<@NonNull UserResponse> findById(@PathVariable UUID id) {
        var userFound = findUserByIdUseCase.execute(id);
        return ResponseEntity.ok(userMapper.userToResponse(userFound));
    }

    @PostMapping
    public ResponseEntity<@NonNull UserResponse> create(@RequestBody @Valid UserPostRequest request) {
